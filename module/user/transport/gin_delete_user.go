package userTransport

import (
	"github.com/gin-gonic/gin"
	"go_service_food_organic/common"
	appContext "go_service_food_organic/component/app_context"
	profileStorage "go_service_food_organic/module/profile/storage"
	userBusiness "go_service_food_organic/module/user/business"
	userRepo "go_service_food_organic/module/user/repository"
	userStorage "go_service_food_organic/module/user/storage"
	"net/http"
)

func GinDeleteUser(appCtx appContext.AppContext) gin.HandlerFunc {
	return func(c *gin.Context) {
		uid, err := common.FromBase58(c.Param("id"))
		if err != nil {
			c.IndentedJSON(http.StatusBadRequest, common.ErrInvalidRequest(err))
		}
		id := int(uid.GetLocalID())

		req := c.MustGet(common.CurrentUser).(common.Requester)

		db := appCtx.GetMyDBConnection()

		storeUser := userStorage.NewSqlModel(db)
		storeProfile := profileStorage.NewSqlModel(db)
		repo := userRepo.NewDeleteUserRepo(storeUser, storeProfile, req)
		biz := userBusiness.NewDeleteUserBiz(repo)

		if err := biz.DeleteUser(c.Request.Context(), id); err != nil {
			panic(err)
		}
		c.IndentedJSON(http.StatusOK, common.SimpleSuccessResponse(true))
	}
}
