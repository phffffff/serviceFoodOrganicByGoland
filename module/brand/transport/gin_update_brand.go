...
func GinUpdateBrand(appCtx appContext.AppContext) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		repo := brandRepo.NewUpdateBrandRepo(store, req)
		biz := brandBusiness.NewUpdateBrandBiz(repo)

		var data brandModel.BrandUpdate

		if err := c.ShouldBind(&data); err != nil {
			panic(err)
		}

		if err := biz.UpdateBrand(c.Request.Context(), id, &data); err != nil {
			panic(err)
		}
